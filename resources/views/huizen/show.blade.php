    ...
    <div class="container mx-auto px-4 py-8">
        ...
        <div class="flex flex-wrap md:flex-nowrap mb-8">
                ...
                <div id="slideshow" class="relative overflow-hidden rounded-lg shadow-lg">
                    @foreach($vakantiehuis->images as $image)
                        <div class="slide {{ $loop->first ? 'active' : '' }}" style="display: {{ $loop->first ? 'block' : 'none' }};">
                            <img src="{{ asset($image->url) }}" alt="{{ $vakantiehuis->naam }}" class="w-full h-80 object-cover">
                        </div>
                    @endforeach
            ...
            <div class="w-full md:w-1/2 lg:w-1/3 bg-white p-6 rounded-lg shadow-md">
                ...
                <p class="text-gray-600 mb-2">{{ $vakantiehuis->straatnaam }} {{ $vakantiehuis->huisnummer }}, {{ $vakantiehuis->stad }}</p>
                <p class="text-green-600 font-semibold text-lg mb-4">€{{ number_format($vakantiehuis->prijs, 2) }} per nacht</p>
        ...
        <div class="bg-white p-6 rounded-lg shadow-md mb-6">
            ...
            <label for="rating" class="block text-gray-700 font-medium">Beoordeling</label>
            <div id="star-rating" class="flex items-center space-x-1">
                <i class="fa fa-star text-gray-300 cursor-pointer" data-value="{{ $recensie->beoordeling }}"></i>
            </div>
            <p class="text-gray-500">{{$recensie->opmerking}}</p>
            @endforeach
